...
> **简单解释一下**： **用户代理样式**：浏览器会有一个基本的样式表来给任何网页设置默认样式。这些样式统称用户代理样式 **页面作者样式**：网页的作者可以定义文档的样式，这是最常见的样式表。大多数情况下此类型样式表会定义多个，它们构成网站的视觉和体验，即页面主题，可以理解为页面作者样式 **用户样式**：读者，作为浏览器的用户，可以使用自定义样式表定制使用体验，自定义用户偏好，可以理解为用户样式
